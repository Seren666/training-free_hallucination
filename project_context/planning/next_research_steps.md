...
> Date: 2026-05-02
> Scope: priority order after full COCO-CHAIR confirmation, evaluator alignment, completed mechanism analysis, and the negative `Object-Safe`, `Attention-Gated`, `Candidate-Local`, and `Middle-Verified` pilots.
...
- every follow-up selective pilot so far underperforms it on the `1000` pilot

## 2. Priority Order

### A. First: consolidate the full negative selective-pilot chain
...
### B. Second: change method family, not just make clipping narrower

If method work continues:

- move away from token-level boost clipping as the default follow-up family
- use the middle-layer audit as analysis evidence, not as a proven runtime gate
...
2. record the four negative selective results clearly:
   - `object_safe_anchor`
...
5. only design a new prototype if it is materially different from token-level boost clipping and whole-caption fallback
...
- future method work should change unit of action, not only narrow the current clipping rule
